...
// src/tag-tree.ts
var normalizeTag2 = (tag) => {
  if (typeof tag !== "string") return [];
  return tag.trim().replace(/^#/, "").split(/[\\/]/).map((part) => part.trim()).filter(Boolean);
};
var sortNodes = (nodes) => {
  nodes.sort((left, right) => left.name.localeCompare(right.name));
  for (const node2 of nodes) {
    node2.pages.sort((left, right) => left.title.localeCompare(right.title));
    sortNodes(node2.children);
  }
};
var buildTagTree = (entries) => {
  const root2 = { name: "", path: "", pages: [], children: [] };
  for (const entry of entries) {
    if (!entry.slug) continue;
    const page = { slug: entry.slug, title: entry.title?.trim() || entry.slug };
    const paths = Array.isArray(entry.tags) ? entry.tags.map(normalizeTag2).filter((parts) => parts.length > 0) : [];
    for (const parts of paths) {
      let current = root2;
      const pathParts = [];
      for (const name of parts) {
        pathParts.push(name);
        let child = current.children.find((node2) => node2.name === name);
        if (!child) {
...
export { ExampleEmitter, ExampleFilter, ExampleTransformer, TagExplorer_default as TagExplorer, buildTagTree };
